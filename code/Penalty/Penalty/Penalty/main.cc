...
int main()
{
	...
	Penalty pen(tail, head, capa, supd, fucl, jacl, 500, 1e-6);
	const clock_t btime = clock();
	vec_d solp = pen.solve();
	double secp = double(clock() - btime) / 1000;
	cout << "The objective value of Pen is: " << std::setprecision(12) << fucl(solp) << "." << endl;
	/*cout << "The violation of cons is: [";
	vec_d fcxp = pen.fcons(solp);
	for (size_t i = 0; i < fcxp.size(); i++) { cout << fcxp[i] << ","; }
	cout << "]." << endl;*/
	cout << "The solving time of Pen is: " << secp << "s." << endl;
	cout << "The number of iterations is: " << pen.track.size() << "." << endl;
	// Output the memory usage of Penalty
	if (GetProcessMemoryInfo(GetCurrentProcess(), &pms, sizeof(pms)))
	{
		cout << "The Memory Usage of Penalty is " << pms.WorkingSetSize / 1024 - memo1 << "." << endl;
	}
	memo1 = pms.WorkingSetSize / 1024;
